#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : nvidia-installer
Version  : 430.14
Release  : 2
URL      : https://github.com/NVIDIA/nvidia-installer/archive/430.14.tar.gz
Source0  : https://github.com/NVIDIA/nvidia-installer/archive/430.14.tar.gz
Summary  : No detailed summary available
Group    : Development/Tools
License  : GPL-2.0
Requires: nvidia-installer-bin = %{version}-%{release}
Requires: nvidia-installer-man = %{version}-%{release}
BuildRequires : libpciaccess-dev
BuildRequires : mesa-dev
BuildRequires : pkgconfig(gl)
BuildRequires : pkgconfig(ncursesw)
Patch1: 0001-set-default-values-to-install-under-opt.patch

%description
nvidia-installer: A tool for installing/un-installing the NVIDIA Linux graphics driver

%package bin
Summary: bin components for the nvidia-installer package.
Group: Binaries

%description bin
bin components for the nvidia-installer package.


%package man
Summary: man components for the nvidia-installer package.
Group: Default

%description man
man components for the nvidia-installer package.


%prep
%setup -q -n nvidia-installer-430.14
%patch1 -p1

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C
export SOURCE_DATE_EPOCH=1559302995
export GCC_IGNORE_WERROR=1
export AR=gcc-ar
export RANLIB=gcc-ranlib
export NM=gcc-nm
export CFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FCFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export FFLAGS="$CFLAGS -O3 -ffat-lto-objects -flto=4 "
export CXXFLAGS="$CXXFLAGS -O3 -ffat-lto-objects -flto=4 "
make  %{?_smp_mflags}


%install
export SOURCE_DATE_EPOCH=1559302995
rm -rf %{buildroot}
%make_install PREFIX=/usr

%files
%defattr(-,root,root,-)

%files bin
%defattr(-,root,root,-)
%exclude /usr/bin/makeself-help-script
%exclude /usr/bin/mkprecompiled
/usr/bin/nvidia-installer

%files man
%defattr(0644,root,root,0755)
/usr/share/man/man1/nvidia-installer.1.gz
